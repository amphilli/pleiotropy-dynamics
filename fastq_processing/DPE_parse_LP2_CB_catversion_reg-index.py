...
BC_REGEX_LIST = [regex.compile('(TCTCTG)(\D{22})(AGTAGA)'),
                 regex.compile('(TCTCTG)(\D{20,24})(AGTAGA)'),
                 regex.compile('(TCTCTG){e<=1}(\D{22})(AGTAGA){e<=1}'),
                 regex.compile('(TCTCTG){e<=1}(\D{20,24})(AGTAGA){e<=1}')]

# lists the inline indices we used that the script should look for
INDEX_LIST = ['GGTATCA','GATGTCCT','CTCATCCAG','GACGGAACTC','ATCGCAGGCAT','TCACGACTAGTA']
...
class BcCounter:
    ...
    def add_entry(self, r1_seq, r1_qual):
        ...
        # Set a bc_start_default as 56 (UMI+distance between inline index and BC)
        bc_start_default = 8 + 48
        ...
        for indi in np.arange(len(INDEX_LIST)):
            if r1_seq[8:15+indi] == INDEX_LIST[indi]:
            ...
            for indi in np.arange(len(INDEX_LIST)):
                ...
                if indi <= 2:
                    inline_regex_list = [regex.compile('('+INDEX_LIST[indi]+'){e<=1}'),
                                         regex.compile('('+INDEX_LIST[indi]+'){e<=2}')
                                        ]
                    for inreg_search in inline_regex_list:
                        inreg_hit = inreg_search.search(r1_seq[8:15+indi])
                        # if fuzzy match works, use the associated index as inline
                        if inreg_hit:
                            inline = INDEX_LIST[indi]
                            break
                # for epochs 6, 8, and 10, allow up to three mismatches
                else:
                    inline_regex_list = [regex.compile('('+INDEX_LIST[indi]+'){e<=1}'),
                                         regex.compile('('+INDEX_LIST[indi]+'){e<=2}'),
                                         #regex.compile('('+INDEX_LIST[indi]+'){e<=3}')
                                        ]
                    for inreg_search in inline_regex_list:
                        inreg_hit = inreg_search.search(r1_seq[8:15+indi])
                        if inreg_hit:
        ...
        if not inline:
            ...
            #elif r1_seq[8:16] == INDEX_LIST[1]:
            #    inline = INDEX_LIST[1]
            #elif r1_seq[8:17] == INDEX_LIST[2]:
            #    inline = INDEX_LIST[2]
            #elif r1_seq[8:18] == INDEX_LIST[3]:
            #    inline = INDEX_LIST[3]
            #elif r1_seq[8:19] == INDEX_LIST[4]:
            #    inline = INDEX_LIST[4]
            #elif r1_seq[8:20] == INDEX_LIST[5]:
            #    inline = INDEX_LIST[5]
            #else:
            #    # don't recognize the inline index
            #    inlinechkr = 1
        ...
        if inlinechkr == 1:
            ...
            bc_start = bc_start_default + len(inline)
            ...
            if np.mean([ord(c)-33 for c in r1_qual[bc_start:bc_start+22]]) < QUALITY_CUTOFF:
                self.quality_count += 1
                    ...
                    # regex check, use a more exact match first, then try a more lenient regexes
                bc = None
                    # looking for barcode
                for reg_search in BC_REGEX_LIST:
                    #CWB changed from AMP's to be +30 instead of +36; might decrease runtime?
                    #Importantly, changed definition of bc below to include the inline index
                    reg_hit = reg_search.search(r1_seq[bc_start-13:bc_start+35])
                    if reg_hit:
                        bc_alone = reg_hit.group(2)
                        bc = inline+'_'+bc_alone
                        break
                if not bc:
                    self.regex_count += 1
                else:
